...
def train_model(X, y): 
    model.w = torch.linalg.lstsq(X, y).solution #<1>
#---
```
...
ax.plot([x[test_point_ix], x[test_point_ix]], [targets[:, test_point_ix].min(), targets[:, test_point_ix].max()],  color='grey', label="Prediction range at test point", zorder = 100,  linestyle='--')
