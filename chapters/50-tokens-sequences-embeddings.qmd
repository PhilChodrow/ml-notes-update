...
# this seemed to really slow down this set of notes, so we're commenting it out for now. 
```
...
```{python}
X_batch, y_batch, X_text_batch, y_text_batch = next(iter(loader))
summary(model, input_size=(X_batch.shape), device=device, dtypes=[torch.long])
```
...
```{python}
gen_text = generate(model, data, init_tokens, context_length = 5, n_tokens = 500, temperature = 0.2)
