...
Let's now begin to break down this problem. 
...
trainer   = BpeTrainer(min_frequency=50)
...
To train a model to perform next-token prediction, we need to organize our sequence of token ids into a data set of training examples, in which each example consists of a sequence of tokens as features and the next token as the target. We can implement this using a custom `Dataset` class in PyTorch.
...
class TextDataSet(Dataset):
    ...
    def __getitem__(self, key):
        target = self.tokens[self.context_length + key]#<2>
        ...
        return feature_tensor, target, feature_text, target_text
    #---
```
...
```{python}
data = TextDataSet(text, trainer = BpeTrainer(min_frequency=50), context_length = 5)
for i in range(5): 
...
Let's add a data loader to make it easier to train our model in batches:
...
```{python}
one_hot_batch(X_batch, m_tokens = data.vocab_length).shape
```



```{python}
# def one_hot(tokens, m_tokens): 
#     return torch.tensor([1 if token == i else 0 for i in range(m_tokens) for token in tokens], dtype = torch.float32)

# def one_hot_batch(batch_tokens, m_tokens): 
#     return torch.stack([one_hot(tokens, m_tokens) for tokens in batch_tokens])
```

The diagram below gives a visual illustration of one-hot encoding. 
...
We're now ready to train a simple neural network to perform next-token prediction with one-hot encoded features. We'll use a simple two-layer neural network for this purpose: 
...
for epoch in range(10):
    ...
    for X_batch, y_batch, _, _ in loader:
        X_one_hot = one_hot_batch(X_batch, m_tokens = data.vocab_length)
        opt.zero_grad()
...
X = one_hot_batch(X.unsqueeze(0), m_tokens = data.vocab_length)
...
```{python}
def deterministic_prediction(preds):
    return torch.argmax(preds, dim = 0).item()
```


In *stochastic prediction*, we treat the scores as defining a probability distribution over the possible next tokens, and we sample from this distribution to get our prediction. The most common choice of probability distribution is the *Boltzmann* distribution, which is defined as follows: the probability that the next token is token $t$ is given by

$$
\begin{aligned}
    p(t) = \frac{e^{s_t / T}}{\sum_{t'} e^{s_{t'} / T}}\;, 
\end{aligned}
$$

where $T> 0 $ is a parameter called the *temperature*. Larger values of $T$ lead to more random predictions, while smaller values of $T$ lead to more deterministic predictions. [Random predictions are sometimes called "creative" among certain enthusiasts.]{.aside} In the limit as $T \to 0$, the Boltzmann distribution converges to a distribution that puts all of its mass on the token with the highest score, which is exactly what deterministic prediction does. In the limit as $T \to \infty$, the Boltzmann distribution converges to the uniform distribution over all tokens, which corresponds to completely random prediction.

```{python}
def boltzmann_prediction(preds, temperature = 1.0):
    probabilities = torch.nn.Softmax()(preds / temperature)
...
for temp in [0.01, 0.5, 1.0, 5.0, 20.0]: 
...
def quick_format(text): 
    return text.replace("  ", "<SPACE>").replace(" ", "").replace("<SPACE>", " ")
...
def generate(model, data, init_tokens, context_length = 5, n_tokens = 200, temperature = 1.0):
    ...
    for _ in range(n_tokens):
        one_hot_tokens = one_hot_batch(context.unsqueeze(0), m_tokens = data.vocab_length)
        scores = model.forward(one_hot_tokens) / temperature
    ...
    decoded = data.tokenizer.decode(generated_tokens.tolist(), skip_special_tokens = True)

    return quick_format(decoded)
```

Let's try it out! At very low temperatures, the model always chooses the most probable next token. This tends to lead to deterministic, repetitive text in which the model gets "stuck" in a loop of predicting the same tokens over and over again. 

```{python}
init_tokens = data[99][0]
context_length = 5

text = generate(model, data, init_tokens, context_length = 5, n_tokens = 100, temperature = 1e-5)
print(text)
```
...
```{python}
text = generate(model, data, init_tokens, context_length = 5, n_tokens = 50, temperature = 1000.0)
print(text)
```

Somewhere in between, we get text that displays occasional flashes of coherence: 

```{python}
text = generate(model, data, init_tokens, context_length = 5, n_tokens = 50, temperature = 1.0)
print(text)
```
...
context_length = 5
model = NTPEmbedding(vocab_size = data.vocab_length, embedding_dim = 2, hidden_size = 200)
loss_fn = torch.nn.CrossEntropyLoss()
opt = torch.optim.Adam(model.parameters(), lr=0.01)
...
```{python}
def generate_boltzman(model, data, init_tokens, context_length = 5, n_tokens = 200, temperature = 1.0):
    generated_tokens = init_tokens
    context = generated_tokens[-context_length:]
    for _ in range(n_tokens):
        scores = model.forward(context.unsqueeze(0))
        # print(scores)
        # print(logits)
        # print(scores)
        y_pred = boltzmann_prediction(scores, temperature = temperature)
...
```{python}
init_tokens = data[100][0]  # add batch dimension
context_length = 5

gen_text = generate_boltzman(model, data, init_tokens, context_length = 5, n_tokens = 100, temperature = 0.01)
...
# scatter = ax.scatter(embeddings[:, 0].numpy(), embeddings[:, 1].numpy(), color = "blue", zorder = 5)

highlight_tokens = ["hamlet", "claudi", "audius", "king", "hamle", "gertrude", "claud", "claudiu", "rosencrantz", "enstern", "horatio", "laertes", "osric", "fortinbras"]

for i, token in enumerate(data.tokenizer.get_vocab().keys()):
    for candidate in highlight_tokens: 
        if candidate in token: 
            color = "red"
            ax.annotate(token, (embeddings[i, 0].item(), embeddings[i, 1].item()), fontsize=12, zorder = 10, color = color)
