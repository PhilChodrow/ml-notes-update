...
An important feature of the gradient is that it tells us the direction in which a small change in the function inputs $\vw$ could produce the greatest increase in the function output $f(\vw)$. Equivalently, the gradient points directly *away* from the direction of greatest decrease in the function output. Here's an example using the function from @exr-linear-gaussian-gradient. `torch` makes it very easy to implement this function. 
...
im = ax.contourf(LL.detach().numpy(), levels=100, cmap='inferno_r', extent = [w1_grid[0], w1_grid[-1], w0_grid[0], w0_grid[-1]])
ax.contour(LL.detach().numpy(), levels=100, colors='black', linewidths=0.5, extent = [w1_grid[0], w1_grid[-1], w0_grid[0], w0_grid[-1]])
...
#---
```
1. Initialize the model with a guess for the parameters $w_0$ and $w_1$. Later, we'll use just a single instance variable which holds a vector of weights. 
2. Method for computing the model's predictions (estimate of the signal) given an input $x$.
...
class GradientDescentOptimizer1D: 
    ...
    def step(self, x, y): 
        ...
        w0_grad, w1_grad = self.grad_func(x, y) #<3>
        # update the parameters
        self.model.w0 = self.model.w0 - self.lr * w0_grad  #<4>
        self.model.w1 = self.model.w1 - self.lr * w1_grad
#---
```
1. Method for computing the gradients of the negative log-likelihood with respect to the parameters $w_0$ and $w_1$, which we did in @exr-linear-gaussian-gradient.
2. Get the gradients. 
3. Update the parameters by taking a step in the direction of the negative gradient, scaled by the learning rate.
