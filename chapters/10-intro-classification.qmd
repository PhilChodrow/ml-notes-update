...
While `torch` implements some bespoke cross-entropy loss functions optimized for various cases, it's also a quick formula to implement by hand: [In the code below we're using the *mean* binary cross-entropy per data point, which corresponds to dividing @eq-cross-entropy by $n$.]{.aside}
...
As with regression, it's possible to use nonlinear feature maps to model nonlinear decision boundaries. For example, here's an implementation using polynomial features: 
...
The resulting decision boundary is more flexible than the linear decision boundary we obtained with the original logistic regression model, although as usual we must content with questions of overfitting and generalization when using more complex models.