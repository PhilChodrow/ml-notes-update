...
#
#
first_query = spectrogram_train_dataset_aug[4]
second_query = spectrogram_train_dataset_aug[4]
...
for epoch in range(10):
    ...
    train_acc, train_loss, cm = evaluate(model, spectrogram_train_loader_aug)
    val_acc, val_loss, cm     = evaluate(model, spectrogram_val_loader)
...
transfer_loader_val = DataLoader(transfer_dataset_val, batch_size=8, shuffle=False)opt = torch.optim.Adam(model.parameters(), lr=1e-3)
...
for epoch in range(5):
    ...
    train_acc, train_loss, cm = evaluate(model, transfer_loader_train)
    val_acc, val_loss, cm     = evaluate(model, transfer_loader_val)
