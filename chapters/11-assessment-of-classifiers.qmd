...
@perdomoDifficultLessonsSocial2025 conducted a recent study of *early warning systems* (EWS) for predicting the likelihood that an individual student will graduate public high school in Wisconsin. The idea of an early warning system is to identify students who are at increased dropout risk so that the school can allocate more resources to supporting those students. 

Typical **features** $\mX$ used in a EWS include information about: 
...
The typical **target** $\vy$ is a binary variable describing whether or not the student drops out of high school rather than graduating on time. A typical **prediction** is a risk score, of which a probability like $\vq$ from our discussion of logistic regression is an example. Schematically, we can write a prediction of the model like this: 

$$
\begin{aligned}
    q(\vx_{\text{student}}, \vx_{\text{environment}})\;,
\end{aligned}
$$
...
In their extensive study, @perdomoDifficultLessonsSocial2025 find some inconclusive evidence that the EWS system used in Wisconsin may have improved graduation rates by approximately 5% among students who received high risk scores, but that evidence was not statistically conclusive. 

#### Environmental Risk Scores and Unnecessary Personalization

A purported feature of $\mathcal{M}_\mathrm{individual}$ is that it produces *personalized*, individual risk scores which aim to give the probability that an individual student will drop out of high school. However, @perdomoDifficultLessonsSocial2025 find that the personalization of $\mathcal{M}_\mathrm{individual}$ is largely unnecessary, writing: 
...
If your classifier is giving you a result that would shock you if you read it in a newspaper or seems like something out of a dystopian scifi novel, then you should ask yourself some careful questions about the design of your study and especially about the structure of your training data. Here's a [case study](https://www.callingbull.org/case_studies/case_study_criminal_machine_learning.html) by Carl Bergstrom and Jevin West about @wu2016automated, a paper which purports to show that people's facial features can be used to predict whether or not those people will commit a crime. 
