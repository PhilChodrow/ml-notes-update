...
We can get a more detailed view of the model architecture and the shapes of the intermediate outputs using the `summary` function from the `torchinfo` package.

```{python}
X, y = next(iter(train_loader))[:2]
...
for i in range(8): 
    axarr.ravel()[i].imshow(X[i].cpu().squeeze(), aspect="auto", origin="lower", zorder = 10, cmap = "inferno")
...
```{python}
X, y = next(iter(spectrogram_train_loader))[:2]
summary(model, input_size=X.shape, device = device)
```
...
for i, ax in enumerate(axarr.flat):
    ...
    im = librosa.display.specshow(spec.cpu().numpy(), y_axis='mel', fmax=8000, x_axis='time', ax = ax, cmap = "inferno")
