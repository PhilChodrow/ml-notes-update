...
Gaussian distribution PDFs for different parameter values.
...
for parameters $w_0, w_1 \in \mathbb{R}$ is called a (1-dimensional) *linear-Gaussian model*. In this context we'll let $\vw = (w_0, w_1)^\top$ denote the parameters of the model, and write $f_\vw(x_i)$ to emphasize the dependence of the signal on the parameters.
...
We are modeling $y_i$ as a noisy sample from a Gaussian distribution centered at the signal value $f_\vw(x_i)$, with noise level $\sigma$ controlling how spread out the distribution is. A larger value of $\sigma$ means that the observed data points $y_i$ will tend to be further away from the signal $f_\vw(x_i)$, while a smaller value of $\sigma$ means that the data points will tend to be closer to the signal. Larger values of the noise level $\sigma$ create noisier data sets where it is more difficult to discern the underlying signal.
...
The linear-Gaussian model is a simple example of a data generating model: it describes a recipe to simulate each target $y_i$ by first computing the signal value $f_\vw(x_i)$, then sampling a noise term $\epsilon_i$ from a Gaussian distribution, and finally adding the two together to get $y_i = f_\vw(x_i) + \epsilon_i$. 
...
$$
\begin{aligned}
    \cL(\vx, \vy; f) = \log L(\vx, \vy; f) = \sum_{i = 1}^n \log p_{y}(y_i; f(x_i), \sigma^2)\;.
\end{aligned}
$${#eq-log-likelihood}

:::

*In this definition we are ignoring the dependence of $L$ and $\cL$ on the noise level $\sigma$ -- that's something that is typically important in statistics contexts but not in predictive ML contexts.*
...
In the context of the linear-Gaussian model specified by @eq-normal-noise and @eq-linear-signal-1d, the likelihood of the observed data is a function of the parameters $\vw = (w_0, w_1)^\top$ of the signal function $f$: 

$$
\begin{aligned}
    L(\vx, \vy; \vw) &= \prod_{i = 1}^n p_{y}(y_i; f_\vw(x_i), \sigma^2)\;. \\ 
    f_\vw(x_i) &= w_0 + w_1 x_i\;.
\end{aligned}
$$

To fit the linear-Gaussian model to data, we'll use the *method of maximum likelihood*, which says that we should choose the parameters $\vw$ to maximize the likelihood of the observed data under the model.

::: {#def-maximum-likelihood}

## Maximum Likelihood Estimation

Given a signal function $f_\vw$ with some parameters $\vw$ (e.g. $\vw = (w_0, w_1)$ in @eq-linear-signal-1d), the *method of maximum likelihood* says that we should choose the parameters $\hat{\vw}$ to maximize the likelihood of the observed data under the model: 

$$
\begin{aligned}
    \hat{\vw} = \argmax_{\vw} \; L(\vx, \vy; \vw)\;.
\end{aligned}
$$
...
$$
\begin{aligned}
    \hat{\vw} = \argmax_{\vw} \; \cL(\vx, \vy; \vw) = \argmin_{\vw} \; -\cL(\vx, \vy; \vw)\;.
\end{aligned}
$$

:::

So, we need to be able to evaluate $-\cL(\vx, \vy; \vw)$ as a function of $\vw$ in order to solve this optimization problem. In case you're wondering, for the Gaussian likelihood this works out to: 

$$
\begin{aligned}
    \cL(\vx, \vy; \vw) &= \sum_{i = 1}^n \log p_{y}(y_i; f_\vw(x_i), \sigma^2) \\ 
    &= \sum_{i = 1}^n \log \left( \frac{1}{\sigma \sqrt{2 \pi}} \exp\left(-\frac{(y_i - f_\vw(x_i))^2}{2\sigma^2}\right) \right) \\ 
    &= \sum_{i = 1}^n \left( -\log(\sigma \sqrt{2 \pi}) - \frac{(y_i - f_\vw(x_i))^2}{2\sigma^2} \right) \\ 
    &= -n \log(\sigma \sqrt{2 \pi}) - \frac{1}{2\sigma^2} \sum_{i = 1}^n (y_i - f_\vw(x_i))^2\;.
\end{aligned}
$$
...
for w0 in torch.linspace(-5, 5, 20):
    for w1 in torch.linspace(-1, 3, 20):
        ...
        ll = gaussian_pdf(y, f(x), sig).log().sum() #<3>
...
3. Compute the data log-likelihood. The `gaussian_pdf` function computes the PDF values for all data points at once, which we then log and sum to get the log-likelihood.
