...
def generate_text(prompt, max_length=50, temperature=1.0, wrap = True):
    ...
    for _ in range(max_length):
        with torch.no_grad():
            output = model(input_ids) #<2>
        next_token_logits = output.logits[:, -1, :] #<3>
        next_token_id = boltzmann_sample(next_token_logits.squeeze(), temperature) #<4>
        generated_tokens.append(next_token_id) #<5>
        input_ids = torch.tensor([generated_tokens])
    
    generated_text = tokenizer.decode(generated_tokens) #<6>
    if wrap:
