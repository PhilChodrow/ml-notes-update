...
As usual, we are going to minimize the negative of @eq-log-likelihood-multinomial, which gives us the multinomial cross-entropy: 

$$
\begin{aligned}
    \mathrm{CE}(\mY, \mX; \vq) = -\sum_{i = 1}^n \sum_{k = 1}^K y_{ik} \log q_k(\vx_i)\;. 
\end{aligned}
$${#eq-cross-entropy}

Here's a quick implementation, assuming: 

- `Y` is a binary $n\times \ell$ matrix in which $y_{ik} = 1$ iff individual $i$ is in class $k$. 
- `Q` is the matrix of signal probabilities, so that $q_{ik} = q_k(\vx_i)$ is the modeled probability that individual $i$ is in class $k$. 
...
In the case of two inputs $\vs = (s_0, s_1)^T$, 

$$
\begin{aligned}
    \softmax(\vs) &= \frac{e^{s_0}}{e^{s_0} + e^{s_1}}\begin{pmatrix}e^{s_0} \\ e^{s_1}\end{pmatrix} = \begin{pmatrix}\frac{e^{s_0}}{e^{s_0} + e^{s_1}} \\ \frac{e^{s_1}}{e^{s_0} + e^{s_1}}\end{pmatrix} \\
                  &= \begin{pmatrix}
                    \frac{1}{1 + e^{s_1 - s_0}} \\ 
                    1 - \frac{1}{1 + e^{s_1 - s_0}}
                  \end{pmatrix} \\ 
                  &= \begin{pmatrix}
                    \sigma(s_1 - s_0) \\ 
                    1 - \sigma(s_1 - s_0)
...
where $\sigma(s) = \frac{1}{1 + e^{-s}}$ is the logistic sigmoid that we've previously studied. This is the way in which the $\softmax$ function generalizes the logistic sigmoid.  
...
where we've used the "." notation to express that the softmax function is applied to each row of the matrix $\mX\mW$: 

$$
\begin{aligned}
    \softmax.(\mX\mW) = \softmax. \begin{pmatrix}
    \vs_1^\top \\
    \vs_2^\top \\
    \vdots \\
    \vs_n^\top
...
# for prediction
x_grid = torch.linspace(-1, 1, 500)
y_grid = torch.linspace(-1, 1, 500)
...
for k in range(3):
    ...
    axarr.ravel()[k].imshow(Q[:, k].reshape(500, 500).detach(), extent=(x_grid.min(), x_grid.max(), y_grid.min(), y_grid.max()), origin='lower', cmap=class_cmap_grad, vmin=0, vmax=1, zorder = 100)
...
axarr.ravel()[3].imshow(colors.reshape(500, 500, 4), extent=(x_grid.min(), x_grid.max(), y_grid.min(), y_grid.max()), origin='lower', zorder = 100)
...
To train the model, we'll need the gradient-descent updates for the entries of $\mW$. The gradient of the log-likelihood with respect to $w_{jk}$ is given by

$$
\begin{aligned}
    \frac{\partial \mathrm{CE}}{\partial w_{jk}} = \sum_{i=1}^n x_{ij} (q_k(\vx_i) - y_{ik})\;.
\end{aligned}
$${#eq-gradient-multinomial}

which can be written in vectorized form as 

$$
\begin{aligned}
    \nabla \mathrm{CE} = \mX^\top (\vq(\mX) - \mY )\;.
\end{aligned}
$${#eq-gradient-multinomial-vectorized}

This gives our gradient descent update rule for the weight matrix $\mW$:
...
Now it's time to train a model. {eq-log-likelihood-multinomial}:
...
def plot_decision_regions(model, X, y, ax):
    cmap = plt.get_cmap('tab20b', 3)
    x_grid = torch.linspace(X[:, 0].min()-2, X[:, 0].max()+2, 500)
    y_grid = torch.linspace(X[:, 1].min()-2, X[:, 1].max()+2, 500)
    ...
    ax.imshow(colors.reshape(500, 500, 4).transpose(1, 0, 2), extent=(x_grid.min(), x_grid.max(), y_grid.min(), y_grid.max()), origin='lower', zorder = 100, aspect='auto')
...
# x_grid = torch.linspace(df['Culmen Length (mm)'].min()-2, df['Culmen Length (mm)'].max()+2, 500)
# y_grid = torch.linspace(df['Culmen Depth (mm)'].min()-2, df['Culmen Depth (mm)'].max()+2, 500)
...
plot_decision_regions(model, X_train, y_train,  axarr[1])
...
plot_decision_regions(model, X_test, y_test, ax)
