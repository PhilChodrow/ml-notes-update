...
$$
\begin{aligned}
    \frac{\partial \cL}{\partial w_{jk}} = \sum_{i=1}^n x_{ij} (q_k(\vx_i) - y_{ik})\;.
\end{aligned}
$$
...
axarr[1].imshow(colors.reshape(500, 500, 4).transpose(1, 0, 2), extent=(x_grid.min(), x_grid.max(), y_grid.min(), y_grid.max()), origin='lower', zorder = 100, aspect='auto')
