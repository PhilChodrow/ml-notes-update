...
Let $h: \R^m \to \R$ be a differentiable function, let $\mA \in \R^{m \times n}$ be a matrix, and let $\vb \in \R^m$ be a vector. Define $g: \R^n \to \R$ by $g(\vv) = h(\mA \vv + \vb)$. Then, for any $\vv_0 \in \R^n$,
...
To apply these identities to @eq-mse-matrix-form, define the function $g:\R^n \rightarrow \R$ by 

$$
\begin{aligned}
    g(\vw) = \frac{1}{n} \norm{\mX \vw - \vy}^2 = \frac{1}{n} h(\mX \vw - \vy)\;.
\end{aligned}
$$
...
$$
\begin{aligned}
    \nabla g(\vw_0) &= \frac{1}{n} \mX^T \nabla h(\mX \vw_0 - \vy) &\quad \text{(Prop 3.2)} \\ 
                    &= \frac{2}{n} \mX^T (\mX \vw_0 - \vy) &\quad \text{(Prop 3.1)}\\
\end{aligned}
$$      
...
model.w.requires_grad_(False) # disable automatic differentiation for the parameters
```
