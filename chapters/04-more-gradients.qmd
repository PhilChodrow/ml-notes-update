...
Let $f: \R^m \to \R$ be a differentiable function, and let $\mA \in \R^{m \times n}$ be a matrix. Define $g: \R^n \to \R$ by $g(\vv) = f(\mA \vv)$. Then, for any $\vv \in \R^n$,
...
```
1. Compute the gradient manually using our `grad_mse` function.
...
def gradient_descent(X, y, w_init, eta, max_iter = int(1e4)):
    ...
    for iteration in range(max_iter):
        loss_history.append(mse(X, y, w_old).item()) #<2>
        w_new = gradient_descent_step(X, y, w_old, eta) #<3>
        if ((w_new - w_old)**2).sum() < 1e-8: #<4>
